...
try:
    # Import from FSOT_Clean_System subdirectory  
    ...
    # Fallback for standalone usage that matches hardwire_fsot signature
...
# Define a type alias to differentiate fallback FSOTCore
if 'fsot_2_0_foundation' not in sys.modules:
    FallbackFSOTCore = FSOTCore
    FSOTCore = FallbackFSOTCore
...
class NeuromorphicNeuralNetwork:
    ...
    def connect_layers(self, 
        ...
        if connection_type == 'full':
            ...
            # Local connections based on spatial proximity
            max_distance = 2.0
            for src_id, src_neuron in source.neurons.items():
                for tgt_id, tgt_neuron in target.neurons.items():

...
try:
    from advanced_dream_engine import AdvancedDreamEngine, get_dream_engine
...
# FSOT system imports - from main directory
...
try:
    from fsot_hardwiring import get_hardwiring_system
    from fsot_2_0_foundation import FSOT20Foundation, get_foundation_system, FSOTState
    FSOT_AVAILABLE = True
    print("[OK] FSOT modules imported successfully")
    
    # Use foundation system as FSOTCore
    class BrainFSOTCoreFoundation:
        """FSOT Core using foundation system."""
        def __init__(self) -> None:
            self.foundation = get_foundation_system()
        
        def compute_universal_scalar(self, d_eff: int = 12, domain: str = 'default', **kwargs: Any) -> float:
            return self.foundation.get_scalar('consciousness_base') or 0.5
            
        def get_fsot_signature(self) -> str:
            return f"FSOT-2.0-{self.foundation.current_state.value.upper()}"
            
        def get_timestamp(self) -> str:
            import time
            return time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    
    # Alias for compatibility - use type ignore to suppress warnings
    FSOTCore = BrainFSOTCoreFoundation  # type: ignore
    
    class BrainFSOTDomain:
        """FSOT Domain using foundation states."""
        NEURAL = "neural"
        COGNITIVE = "cognitive"
        CONSCIOUSNESS = "consciousness"
    
    # Alias for compatibility - use type ignore to suppress warnings   
    FSOTDomain = BrainFSOTDomain  # type: ignore
        
except ImportError as e:
    print(f"⚠️ FSOT import failed, using fallback: {e}")
    imported_fsot_enforce = None
    
    class BrainFSOTCoreFallback:
    ...
    FSOTCore = BrainFSOTCoreFallback  # type: ignore

    class FallbackFSOTDomain:
        """Fallback FSOTDomain implementation."""
        NEURAL = "neural"
        COGNITIVE = "cognitive"
        CONSCIOUSNESS = "consciousness"
    
    FSOTDomain = FallbackFSOTDomain  # type: ignore
...
class NeuromorphicBrainSystem:
    ...
    def _process_fractal_dream(self, scenario: Dict[str, Any], consciousness: float) -> Dict[str, Any]:
        ...
        try:
            ...
            fractal_patterns = self.dream_engine.analyze_web_image_fractals([search_query])
            
            return {
                'dream_type': 'fractal_analysis',
                'fractal_patterns_found': len(fractal_patterns.get('pattern_types', [])),
                'patterns': [
                    {
                        'type': pattern_type,
                        'complexity': fractal_patterns.get('complexity_score', 0.5),
                        'dimension': fractal_patterns.get('fractal_dimension', 1.5),
                        'confidence': fractal_patterns.get('confidence', 0.7)
                    } for pattern_type in fractal_patterns.get('pattern_types', [])[:5]  # Top 5 patterns
                ],
    ...
    def _process_artistic_dream(self, scenario: Dict[str, Any], consciousness: float) -> Dict[str, Any]:
        ...
        try:
            if not self.dream_engine:
                return {'dream_error': 'Dream engine not available'}
                
            # Enter dream state
            dream_state = self.dream_engine.enter_dream_state(consciousness, scenario.get('content', 'creative_vision'))
            ...
            image_urls = [search_query]  # Simplified - using search query as placeholder
    ...
    def _process_exploration_dream(self, scenario: Dict[str, Any], consciousness: float) -> Dict[str, Any]:
        ...
        try:
            ...
            # Enter dream state
            dream_state = self.dream_engine.enter_dream_state(consciousness, scenario.get('content', 'exploration'))
    ...
    def _process_perception_dream(self, scenario: Dict[str, Any], consciousness: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if frame is not None:
                ...
                dream_state = self.dream_engine.enter_dream_state(consciousness, scenario.get('content', 'perception'))
                outcomes = self.dream_engine.simulate_dream_outcomes(dream_state)
                dream_summary = self.dream_engine.exit_dream_state(dream_state.dream_id)
                ...
                return {
                    ...
                    'frame_analysis': {
                        'complexity': scenario.get('frame_complexity', 0.5),
                        'patterns': patterns.get('detected_patterns', [])[:3]
                    },
    ...
    def _process_general_dream(self, scenario: Dict[str, Any], consciousness: float) -> Dict[str, Any]:
        ...
        try:
            ...
            # Enter dream state
            dream_state = self.dream_engine.enter_dream_state(consciousness, scenario.get('content', 'general_exploration'))
